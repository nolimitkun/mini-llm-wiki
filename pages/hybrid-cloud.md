...
- **Consistent substrate**: Kubernetes + object storage (S3-compatible, e.g. MinIO on-prem) + **open [table formats](table-and-file-formats.md)** as the portability layer — the same Iceberg/Delta tables and engines run both sides, making placement a policy decision instead of a rewrite. The [open source stack](open-source-data-ai-platform.md) is the fullest expression of this pattern.
